...
pub mod lssvm_pair_missing_enumerable {
    ...
    pub static LSSVMPAIRMISSINGENUMERABLE_ABI: ::ethers::contract::Lazy<
        ::ethers::core::abi::Abi,
    ...
    for LSSVMPairMissingEnumerableErrors {
    ...
    impl ::ethers::contract::EthLogDecode for LSSVMPairMissingEnumerableEvents {
        fn decode_log(
            ...
            if let Ok(decoded) = AssetRecipientChangeFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::AssetRecipientChangeFilter(decoded),
                );
            }
            ...
            if let Ok(decoded) = NftwithdrawalFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::NftwithdrawalFilter(decoded),
                );
            }
            if let Ok(decoded) = OwnershipTransferredFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::OwnershipTransferredFilter(decoded),
                );
            }
            if let Ok(decoded) = SpotPriceUpdateFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::SpotPriceUpdateFilter(decoded),
                );
            }
            if let Ok(decoded) = SwapNFTInPairFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::SwapNFTInPairFilter(decoded),
                );
            }
            if let Ok(decoded) = SwapNFTOutPairFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::SwapNFTOutPairFilter(decoded),
                );
            }
            if let Ok(decoded) = TokenDepositFilter::decode_log(log) {
                return Ok(LSSVMPairMissingEnumerableEvents::TokenDepositFilter(decoded));
            }
            if let Ok(decoded) = TokenWithdrawalFilter::decode_log(log) {
                return Ok(
                    LSSVMPairMissingEnumerableEvents::TokenWithdrawalFilter(decoded),
                );
            }
    ...
    for LSSVMPairMissingEnumerableEvents {
    ...
    for LSSVMPairMissingEnumerableCalls {
