...
mod tests {
    ...
    use crate::{EchoExecutor, Event, FiberCollector, MevBundle, StreamType};

    #[tokio::test]
    pub async fn test_chainbound_clients() {
        if let Ok(api_key) = std::env::var("FIBER_TEST_KEY") {
            ...
            let provider = std::sync::Arc::new(
                ethers::providers::Provider::connect("https://eth.llamarpc.com")
                    .await
                    .unwrap(),
            );
            let tx_signer = ethers::signers::LocalWallet::new(&mut rand::thread_rng());
            let auth_signer = ethers::signers::LocalWallet::new(&mut rand::thread_rng());
            ...
            let action = MevBundle::with_txs(vec![tx.into()]);
            echo_executor.execute(action).await.unwrap();
        } else {
